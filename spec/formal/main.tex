...
\begin{itemize}
    ...
    \item Finally, set $\sem{\texttt{for}\, v,\{ M_1 := e_1', \ldots, M_m := e_m' \}(e_d, e_1, \ldots, e_m)}{\I}:= I(M_1)$.
\end{itemize}

TODO: Typing rule

\subsection{Pointwise Functions}
The syntax definition of $\ML$ assumes a repertoire $\Omega$ of functions $f : \C^n \rightarrow \C^n$, representing all possible pointwise functions that can be used with $\Apply$.
Here $\C$ is a semiring, often the complex or real numbers.
...
\begin{tabbing}
    ...
    \>\>\>$\Apply[y, d, p.\ \SyntaxStyle{if}\ d = \mathbf{0}\ \SyntaxStyle{then}\ p\ \SyntaxStyle{else}\ y](Y, D, \transp{P})))^*$\\
\end{tabbing}
