...
\item $\ttype(\Apply[f](e_1,\ldots ,e_k)):= (\alpha,\beta)$, if $\ttype(e_i) = (\alpha, \beta)$ for all $i := 1,\ldots,k$.
\end{itemize}
...
We define a function $\dom : \DD \to \NN$ which maps size symbols to sizes, 
...
We denote the evaluation of an expression $e$ over instance $I$ as follows:
