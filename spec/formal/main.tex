...
\textbf{TODO: Needs updating to a simpler definition}
...
\begin{tabbing}
    L0\=L1\=L2\=L3\=L4 \kill 
    $\texttt{for}\, v,X \texttt{.}\,$\\ 
    \>$\SyntaxStyle{let}\ B = \transp{v} \cdot A \ \SyntaxStyle{in}$ \\
    \>$\Apply[+]($\\
    \>\>$X,$\\
    \>\>$(\texttt{for}\, w,Y \texttt{.}\,$\\ 
    \>\>\>$\SyntaxStyle{let}\ D=\one{Y} \cdot \transp{\one{Y}} \cdot Y\ \SyntaxStyle{in}$ \\
    \>\>\>$\SyntaxStyle{let}\ P=\diag(w) \cdot \transp{B}\ \SyntaxStyle{in}$ \\
    \>\>\>$\Apply[y, d, p.\ \SyntaxStyle{if}\ d = 0\ \SyntaxStyle{then}\ p\ \SyntaxStyle{else}\ y](Y, D, \transp{P})))^T$\\
    \>\>$)^T$\\
    %\>\>\>$\SyntaxStyle{let}\ D=Y \cdot \one(\transp{Y}) \cdot \transp{\one(\transp{Y})}\ \SyntaxStyle{in}$ \\
    %\>\>\>$\SyntaxStyle{let}\ D=\transp{\one(Y)} \cdot \one(Y) \cdot Y\ \SyntaxStyle{in}$ \\
    %\>\>\>$\SyntaxStyle{let}\ P=\diag(v) \cdot A \cdot \diag(w)\ \SyntaxStyle{in}$ \\
