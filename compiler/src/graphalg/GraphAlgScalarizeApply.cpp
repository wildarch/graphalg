 ...
 */
class GraphAlgScalarizeApply : public impl::GraphAlgScalarizeApplyBase<GraphAlgScalarizeApply> {
public:
    using impl::GraphAlgScalarizeApplyBase<GraphAlgScalarizeApply>::GraphAlgScalarizeApplyBase;
...
mlir::LogicalResult OpConversion<CastOp>::matchAndRewrite(
        ...
        OpAdaptor adaptor,
        mlir::ConversionPatternRewriter& rewriter) const {
    rewriter.replaceOpWithNewOp<CastScalarOp>(
            op,
            op.getType().getSemiring(),
            adaptor.getInput());
    return mlir::success();
}
...
static mlir::LogicalResult unrollLoopBody(
        mlir::Operation* loopOp,
        mlir::Block& body,
        const llvm::APInt& iter,
    ...
    return loopOp->emitOpError("does not have a terminator ")
            ...
            << ", so cannot be inlined";
}

static mlir::LogicalResult unrollForDimOne(
        ForDimOp op,
...
static mlir::LogicalResult unrollForConst(
        ForConstOp op,
        mlir::PatternRewriter& rewriter) {
...
mlir::FailureOr<mlir::Value> createScalarOpFor(
    ...
    case BinaryOp::SUB:
        if (type.isIntOrIndex()) {
            ...
            return mlir::Value(
                    builder.create<mlir::arith::SubFOp>(loc, lhs, rhs));
        } else {
            return mlir::emitError(loc)
                    << "subtraction is not defined for values of type "
                    << type;
        }
    case BinaryOp::MUL:
    ...
    case BinaryOp::DIV:
        if (type.isF64()) {
            return mlir::Value(
                    builder.create<mlir::arith::DivFOp>(loc, lhs, rhs));
        } else {
            return mlir::emitError(loc)
                    << "division is not defined for values of type "
                    << type;
        }
    case BinaryOp::EQ:
        return mlir::Value(builder.create<EqOp>(loc, lhs, rhs));
    case BinaryOp::NE: {
        auto falseOp = builder.create<ConstantOp>(
                loc,
...
static mlir::Value applyMask(
    ...
    return builder.create<ElementWiseOp>(
            mask.getLoc(),
            castOp,
            BinaryOp::MUL,
            value);
...
static mlir::LogicalResult convertMatMul(
        MatMulOp op,
        mlir::PatternRewriter& rewriter) {
...
static mlir::LogicalResult convertNeg(
    ...
    auto zeroOp = rewriter.create<LiteralOp>(
            op->getLoc(),
            sring.addIdentity());
    rewriter.replaceOpWithNewOp<ElementWiseOp>(
            op,
...
static mlir::LogicalResult convertNot(
        NotOp op,
        mlir::PatternRewriter& rewriter) {
    auto falseOp = rewriter.create<LiteralOp>(
            op->getLoc(),
...
static mlir::LogicalResult convertNVals(
    ...
    auto sring = llvm::cast<SemiringTypeInterface>(
            op.getInput().getType().getSemiring());
    auto zeroOp = rewriter.create<LiteralOp>(
            op->getLoc(),
            sring.addIdentity());
    auto neOp = rewriter.create<ElementWiseOp>(
            op->getLoc(),
            zeroOp,
            BinaryOp::NE,
            op.getInput());
...
void GraphAlgScalarizeApply::runOnOperation() {
    ...
    conversions.add<
            OpConversion<ApplyInlineReturnOp>,
            OpConversion<CastOp>,
            OpConversion<ConstantMatrixOp>,
            OpConversion<ElementWiseOp>,
            OpConversion<LiteralOp>,
            OpConversion<ApplyOp>>(typeConverter, &getContext());
    ...
    conversions.add<OpConversion<ApplyInlineOp>>(applyTypeConverter, &getContext());

    // Convert only ApplyInlineOp and their bodies.
    llvm::SmallVector<ApplyInlineOp> applyOps;
    getOperation().walk([&](ApplyInlineOp op){
