...
bool parser<graphalg::CallArgumentDimensions>::parse(
    ...
    auto err = [&]() {
        return o.error(
                "Call argument dimensions '" + arg + "' are invalid",
                argName);
    };
 ...
 */
class GraphAlgSetDimensions : public impl::GraphAlgSetDimensionsBase<GraphAlgSetDimensions> {
    using impl::GraphAlgSetDimensionsBase<GraphAlgSetDimensions>::GraphAlgSetDimensionsBase;
...
public:
    static mlir::FailureOr<DimMapper> build(
            mlir::func::FuncOp func,
            llvm::ArrayRef<CallArgumentDimensions> args);

    mlir::LogicalResult tryMap(
            mlir::Location loc,
            DimAttr sym,
            std::uint64_t dim);
...
public:
    DimConversionPattern(
            const DimMapperTypeConverter& converter,
...
mlir::FailureOr<DimMapper> DimMapper::build(
        mlir::func::FuncOp func,
        llvm::ArrayRef<CallArgumentDimensions> args) {
...
mlir::LogicalResult DimMapper::tryMap(
        mlir::Location loc,
        DimAttr sym,
        std::uint64_t dim) {
    if (sym.isConcrete() && sym.getConcreteDim() != dim) {
        return mlir::emitError(loc)
                << "Concrete dimension "
                << sym
                << " must be mapped to itself, but got "
                << dim;
    }
    ...
    if (!newlyAdded && it->second != dim) {
        return mlir::emitError(loc)
                << "Attempt to map abstract dimension "
                << sym
                << " to concrete dimension "
                << dim
...
template<>
mlir::LogicalResult
OpConversion<mlir::func::FuncOp>::matchAndRewrite(
        mlir::func::FuncOp op,
        OpAdaptor adaptor,
    ...
    if (mlir::failed(typeConverter->convertTypes(
            funcType.getInputs(),
            inputTypes))) {
...
mlir::LogicalResult DimConversionPattern::matchAndRewrite(
    ...
    rewriter.modifyOpInPlace(op, [&]() {
        ...
        // Update result types
        for (auto [res, newType] : llvm::zip_equal(
                op->getResults(),
                newResultTypes)) {
            res.setType(newType);
        }
...
void GraphAlgSetDimensions::runOnOperation() {
    ...
    target.addDynamicallyLegalDialect<GraphAlgDialect>(doesNotUseAbstractDimensions);
    target.addDynamicallyLegalOp<mlir::func::ReturnOp>(doesNotUseAbstractDimensions);
    ...
    patterns.add<OpConversion<mlir::func::FuncOp>>(
            typeConverter,
            &getContext());
    ...
    patterns.add<
            DimOpRewritePattern<CastDimOp>,
            DimOpRewritePattern<ForDimOp>>(*dimMapper, &getContext());
