...
#include "graphalg/GraphAlgOpsDialect.cpp.inc"
...
mlir::Operation* GraphAlgDialect::materializeConstant(
    ...
    if (auto typedValue = llvm::dyn_cast<mlir::TypedAttr>(value)) {
        if (auto matrix = llvm::dyn_cast<MatrixType>(type)) {
            if (typedValue.getType() == matrix.getSemiring()) {
                return builder.create<ConstantMatrixOp>(
                        loc,
                        matrix,
                        typedValue);
            }
    ...
    mlir::emitError(loc)
            << "Unable to materialize constant value "
            << value
            << " of type "
            << type;
    return nullptr;
...
mlir::TypedAttr GraphAlgDialect::castAttribute(
        mlir::TypedAttr attr,
        mlir::Type to) const {
