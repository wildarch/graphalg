 ...
 */
class GraphAlgPrepareInline : public impl::GraphAlgPrepareInlineBase<GraphAlgPrepareInline> {
private:
    mlir::LogicalResult verifyAcyclic(const mlir::CallGraph& graph);

public:
    using impl::GraphAlgPrepareInlineBase<GraphAlgPrepareInline>::GraphAlgPrepareInlineBase;
...
struct ApplyAdaptor {
    ...
    mlir::Value populateApply(
            mlir::ValueRange bodyArgs,
            mlir::PatternRewriter& rewriter);
    mlir::LogicalResult replaceWithApply(
            mlir::Operation* op,
            mlir::PatternRewriter& rewriter);
};
...
mlir::Value ApplyAdaptor::populateApply(
        mlir::ValueRange bodyArgs,
        mlir::PatternRewriter& rewriter) {
    if (isSelect) {
        ...
        auto callOp = rewriter.create<mlir::func::CallOp>(
                ...
                func,
                MatrixType::scalarOf(
                        SemiringTypes::forBool(rewriter.getContext())),
                bodyArgs);
        ...
        auto mulOp = rewriter.create<ElementWiseOp>(
                loc,
                castOp,
                BinaryOp::MUL,
                arg0);
...
mlir::LogicalResult ApplyAdaptor::replaceWithApply(
        mlir::Operation* op,
        mlir::PatternRewriter& rewriter) {
...
static mlir::LogicalResult convertApplyUnary(
        ApplyUnaryOp op,
        mlir::PatternRewriter& rewriter) {
    return ApplyAdaptor{
        .loc = op->getLoc(),
...
static mlir::LogicalResult convertApplyBinary(
        ApplyBinaryOp op,
        mlir::PatternRewriter& rewriter) {
    return ApplyAdaptor{
        .loc = op->getLoc(),
...
static mlir::LogicalResult convertSelectUnary(
        SelectUnaryOp op,
        mlir::PatternRewriter& rewriter) {
    return ApplyAdaptor{
        .loc = op->getLoc(),
...
static mlir::LogicalResult convertSelectBinary(
        SelectBinaryOp op,
        mlir::PatternRewriter& rewriter) {
    return ApplyAdaptor{
        .loc = op->getLoc(),
...
static mlir::LogicalResult convertApplyElementWise(
        ApplyElementWiseOp op,
        mlir::PatternRewriter& rewriter) {
    return ApplyAdaptor{
        .loc = op->getLoc(),
        .isSelect = false,
...
mlir::LogicalResult GraphAlgPrepareInline::verifyAcyclic(
        const mlir::CallGraph& graph) {
