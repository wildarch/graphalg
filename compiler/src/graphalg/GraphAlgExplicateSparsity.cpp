 ...
 */
class GraphAlgExplicateSparsity : public impl::GraphAlgExplicateSparsityBase<GraphAlgExplicateSparsity> {
    using impl::GraphAlgExplicateSparsityBase<GraphAlgExplicateSparsity>::GraphAlgExplicateSparsityBase;
...
static mlir::FailureOr<ApplyOp> cloneUpTo(
        ApplyOp op,
        mlir::Value target,
        bool negateTarget,
    ...
    auto newOp = rewriter.create<ApplyOp>(
            op->getLoc(),
            op.getType().withSemiring(target.getType()),
            op.getInputs());
    ...
    if (!newTarget) {
        op->emitOpError("attempt to clone until value")
                << target
                << ", but it is not produced by this op";
    }
...
static mlir::LogicalResult extractElementWiseAdd(
        ApplyOp op,
        mlir::PatternRewriter& rewriter) {
    auto returnOp = llvm::cast<ApplyReturnOp>(
...
static MakeDenseOp makeDense(
    ...
    rewriter.replaceOpWithNewOp<MakeDenseReturnOp>(
            returnOp,
            returnOp.getValue());
...
static void makeInputsDense(
    ...
    rewriter.modifyOpInPlace(op, [&]() {
        op.getInputsMutable().assign(denseInputs);
    });
}

static bool mayHaveSparseInputs(
        ApplyOp op,
