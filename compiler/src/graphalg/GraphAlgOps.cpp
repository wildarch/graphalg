...
mlir::LogicalResult TransposeOp::inferReturnTypes(
    ...
    auto inputType = llvm::cast<MatrixType>(
...
mlir::LogicalResult DiagOp::inferReturnTypes(
    ...
    auto dim = inputType.getRows().isOne()
            ? inputType.getCols()
            : inputType.getRows();
...
// result type.
static MatrixType buildApplyResultType(
    ...
    auto funcResultType = llvm::cast<MatrixType>(
            func.getFunctionType().getResult(0));

    return MatrixType::get(
            func->getContext(),
            firstInputType.getRows(),
            firstInputType.getCols(),
            funcResultType.getSemiring());
}

static mlir::LogicalResult verifyCalleeSignature(
        mlir::SymbolTableCollection& symbolTable,
        mlir::CallOpInterface callOp,
        mlir::FunctionType expected) {
    auto callable = llvm::dyn_cast_if_present<mlir::func::FuncOp>(
...
void ApplyUnaryOp::build(
    ...
    build(
            builder,
            state,
            buildApplyResultType(func, input),
            func.getSymName(),
            input);
}

mlir::LogicalResult ApplyUnaryOp::verifySymbolUses(
        mlir::SymbolTableCollection& symbolTable) {
    auto expectedFunctionType = mlir::FunctionType::get(
            getContext(),
            getInput().getType().asScalar(),
            getType().asScalar());
...
static mlir::FlatSymbolRefAttr castToFunctionRef(
        mlir::CallInterfaceCallable callee) {
    return llvm::cast<mlir::FlatSymbolRefAttr>(
            callee.get<mlir::SymbolRefAttr>());
}
...
void ApplyBinaryOp::build(
    ...
    build(
            builder,
            state,
            buildApplyResultType(func, lhs),
            func.getSymName(),
            lhs,
            rhs);
}

mlir::LogicalResult ApplyBinaryOp::verifySymbolUses(
        mlir::SymbolTableCollection& symbolTable) {
    auto expectedFunctionType = mlir::FunctionType::get(
            getContext(),
            std::array<mlir::Type, 2>{
                getLhs().getType().asScalar(),
...
mlir::LogicalResult ApplyElementWiseOp::verifySymbolUses(
        mlir::SymbolTableCollection& symbolTable) {
    auto expectedFunctionType = mlir::FunctionType::get(
            getContext(),
            std::array<mlir::Type, 2>{
...
// === SelectUnaryOp ===
mlir::LogicalResult SelectUnaryOp::verifySymbolUses(
        mlir::SymbolTableCollection& symbolTable) {
    auto expectedFunctionType = mlir::FunctionType::get(
            getContext(),
            getInput().getType().asScalar(),
...
// === SelectBinaryOp ===
mlir::LogicalResult SelectBinaryOp::verifySymbolUses(
        mlir::SymbolTableCollection& symbolTable) {
...
mlir::LogicalResult MatMulOp::inferReturnTypes(
        ...
        Adaptor adaptor,
        llvm::SmallVectorImpl<mlir::Type>& inferredReturnTypes) {
    inferredReturnTypes.emplace_back(inferMatMulReturnType(
            adaptor.getLhs(),
            adaptor.getRhs()));
    return mlir::success();
}
...
// === ReduceOp ===
static mlir::LogicalResult verifyCollapseDim(
        ReduceOp op,
    ...
    if (!resultDim.isOne() && resultDim != inputDim) {
        op->emitOpError("result ")
                ...
                << " "
                << resultDim
                << " are not collapsed to 1 and do not match the input "
                << dimName
                << " "
                << inputDim;
    }
...
mlir::LogicalResult ReduceOp::verify() {
    ...
    if (mlir::failed(verifyCollapseDim(
            *this,
            "rows",
            getType().getRows(),
            inputType.getRows()))) {
...
// === CastOp ===
static mlir::LogicalResult verifyCast(
        mlir::Operation* op,
        mlir::Type from,
        mlir::Type to) {
...
mlir::LogicalResult verifyLoop(
    ...
    for (auto& region : op->getRegions()) {
        ...
        if (argTypes.empty() || argTypes.front() != iterType) {
            return op->emitOpError("region types ")
                    << argTypes
                    << "do not include the iteration variable";
        }
    }
    ...
    auto bodyYield = llvm::dyn_cast_if_present<YieldOp>(
            body.getTerminator());
    if (!bodyYield) {
    ...
    if (!untilRegion.empty()) {
        ...
        auto untilYield = llvm::dyn_cast_if_present<YieldOp>(
                until.getTerminator());
        if (!untilYield) {
            return op->emitOpError(
                    "until region is not terminated with a YieldOp");
        }

        auto expectedType = MatrixType::scalarOf(SemiringTypes::forBool(ctx));
        if (untilYield->getOperandTypes() != mlir::TypeRange{ expectedType }) {
            return op->emitOpError(
                    "until block does not return a bool scalar: ")
...
void getLoopSuccessorRegions(
    ...
    if (point.isParent()) {
        ...
        regions.emplace_back(
                mlir::RegionSuccessor(op->getResults()));
        // Or into the body
...
mlir::OperandRange ForConstOp::getEntrySuccessorOperands(
        mlir::RegionBranchPoint point) {
    return getInitArgs();
}
...
mlir::OperandRange ForDimOp::getEntrySuccessorOperands(
        mlir::RegionBranchPoint point) {
    return getInitArgs();
}
...
void ApplyInlineOp::build(
    ...
    build(
            builder,
            state,
            firstInputType.withSemiring(semiring),
            inputs);
...
static mlir::LogicalResult verifyInputDimensionsMatchOutput(
        mlir::Operation* op,
        MatrixType output,
        mlir::ValueRange inputs) {
    ...
    for (auto [i, input] : llvm::enumerate(inputs)) {
        ...
        if (inputType.getDims() != dims) {
            return op->emitOpError("is expecting all inputs to have the same ")
                    << "dimensions as the result, but the input at index "
                    << i
                    << " has type "
                    << inputType
                    << " (expected dimensions to match "
                    << output
                    << ")";
        }
    }
...
mlir::LogicalResult ApplyInlineOp::verify() {
    if (mlir::failed(verifyInputDimensionsMatchOutput(
            *this,
            getType(),
            getInputs()))) {
        return mlir::failure();
    ...
    for (auto [i, input, arg] : llvm::enumerate(getInputs(), args)) {
        ...
        if (!argType || !argType.isScalar()) {
            return emitOpError(" is expecting all body arguments to be scalar ")
                    << "matrices, but the argument at index "
                    << i
                    << " is of type "
                    << argType;
        }
...
mlir::LogicalResult ApplyInlineOp::verifyRegions() {
    ...
    auto returnOp = llvm::dyn_cast_if_present<ApplyInlineReturnOp>(
            body.getTerminator());
    if (!returnOp) {
    ...
    for (auto input : getInputs()) {
        ...
        if (inRows != outRows && !inRows.isOne()) {
            auto diag = emitOpError(
                    "input and output row dimensions are incompatible");
            diag.attachNote(input.getLoc())
                    << "incompatible input is "
                    << input;
        }

        if (inCols != outCols && !inCols.isOne()) {
            auto diag = emitOpError(
                    "input and output column dimensions are incompatible");
            diag.attachNote(input.getLoc())
                    << "incompatible input is "
                    << input;
        }
    ...
    for (auto [i, input, arg] : llvm::enumerate(getInputs(), args)) {
        ...
        if (!llvm::isa<SemiringTypeInterface>(argType)) {
            return emitOpError(" is expecting all body arguments to have ")
                    << "semiring types, but the argument at index "
                    << i
                    << " is of type "
                    << argType;
        }
...
mlir::LogicalResult ApplyOp::verifyRegions() {
    ...
    auto returnOp = llvm::dyn_cast_if_present<ApplyReturnOp>(
            body.getTerminator());
    if (!returnOp) {
        return emitOpError("body is not terminated with ")
...
mlir::LogicalResult MakeDenseOp::verifyRegions() {
    ...
    auto returnOp = llvm::dyn_cast_if_present<MakeDenseReturnOp>(
            body.getTerminator());
    if (!returnOp) {
        return emitOpError("body is not terminated with ")
...
static mlir::LogicalResult verifyUnionOperandType(mlir::Operation* op) {
    auto resultType = llvm::cast<MatrixType>(
...
mlir::LogicalResult UnionOp::verify() {
