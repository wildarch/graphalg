...
static mlir::LogicalResult matMulScalar(
        MatMulOp op,
        mlir::PatternRewriter& rewriter) {
    if (!op.getLhs().getType().isScalar()
            || !op.getRhs().getType().isScalar()) {
        return mlir::failure();
    }
...
static bool isMulIdentityMatrix(mlir::Value v) {
    ...
    auto sring = llvm::cast<SemiringTypeInterface>(
            constOp.getType().getSemiring());
    return constOp.getValue() == sring.mulIdentity();
}

static mlir::LogicalResult matMulBroadcast(
        MatMulOp op,
        mlir::PatternRewriter& rewriter) {
...
mlir::OpFoldResult CastDimOp::fold(FoldAdaptor adaptor) {
    if (getInput().isConcrete()) {
        return mlir::IntegerAttr::get(
                SemiringTypes::forInt(getContext()),
...
static mlir::LogicalResult applyConstantInput(
        ApplyOp op,
        mlir::PatternRewriter& rewriter) {
...
mlir::LogicalResult inlineApply(
        ApplyOp consumer,
        ApplyOp producer,
    ...
    rewriter.inlineBlockBefore(
            &consumer.getBody().front(),
            &newBody,
            newBody.end(),
            consumerArgReplacements);
...
mlir::OpFoldResult LiteralOp::fold(FoldAdaptor adaptor) {
    return getValue();
}
...
static mlir::LogicalResult forDimConst(
        ForDimOp op,
        mlir::PatternRewriter& rewriter) {
    ...
    // Range from 0 to dim.
    auto intType = MatrixType::scalarOf(
            SemiringTypes::forInt(rewriter.getContext()));
...
void ForDimOp::getCanonicalizationPatterns(
        mlir::RewritePatternSet& patterns,
        mlir::MLIRContext* context) {
...
static mlir::LogicalResult pickAnyReduceBool(
        PickAnyOp op,
        mlir::PatternRewriter& rewriter) {
    ...
    // Relax to a union
    auto unionOp = rewriter.createOrFold<UnionOp>(
            op.getLoc(),
            reduceOp.getType(),
            reduceOp.getInputs());
    rewriter.modifyOpInPlace(op, [&]() {
        op.getInputMutable().assign(unionOp);
    });

    return mlir::failure();
}

void PickAnyOp::getCanonicalizationPatterns(
        mlir::RewritePatternSet& patterns,
        mlir::MLIRContext* context) {
...
mlir::OpFoldResult CastScalarOp::fold(FoldAdaptor adaptor) {
    ...
    // Constant folding.
    if (auto typedAttr = llvm::dyn_cast_if_present<mlir::TypedAttr>(
            adaptor.getInput())) {
...
mlir::OpFoldResult UnionOp::fold(FoldAdaptor adaptor) {
    ...
    bool hasUnionInputs = llvm::any_of(getInputs(), [](mlir::Value v) {
        return v.getDefiningOp<UnionOp>();
    });
    if (hasUnionInputs) {
