...
mlir::LogicalResult TropInfAttr::verify(
    ...
    if (!llvm::isa<SemiringTypeInterface>(type)) {
        ...
        return emitError()
                << TropInfAttr::name
                << " is not an element of the semiring "
                << type;
    }
...
mlir::LogicalResult TropIntAttr::verify(
        llvm::function_ref<mlir::InFlightDiagnostic()> emitError,
        mlir::Type type,
...
mlir::LogicalResult TropFloatAttr::verify(
        llvm::function_ref<mlir::InFlightDiagnostic()> emitError,
        mlir::Type type,
...
DimAttr DimAttr::getConcrete(
        mlir::MLIRContext* context,
        std::uint64_t dim) {
    return Base::get(context, /*id=*/nullptr, dim);
}

bool DimAttr::isAbstract() const {
    return getAbstractId() != nullptr;
}

bool DimAttr::isConcrete() const {
    return getAbstractId() == nullptr;
}

bool DimAttr::isOne() const {
    return isConcrete() && getConcreteDim() == 1;
}
