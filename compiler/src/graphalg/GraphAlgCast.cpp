...
static mlir::FloatAttr convertIntegerAttrToFloat(mlir::IntegerAttr attr) {
    ...
    realVal.convertFromAPInt(
            attr.getValue(),
            /*isSigned=*/true,
            llvm::RoundingMode::NearestTiesToEven);
    return mlir::FloatAttr::get(SemiringTypes::forReal(attr.getContext()), realVal);
}

static mlir::TypedAttr intToReal(
        mlir::TypedAttr attr,
...
static mlir::TypedAttr tropIntToTropReal(
        mlir::TypedAttr attr,
        SemiringTypeInterface inRing,
        SemiringTypeInterface outRing) {
    ...
    return TropFloatAttr::get(
            ctx,
            outRing,
            convertIntegerAttrToFloat(intAttr));
}
...
static mlir::IntegerAttr truncateFloatAttrToInteger(mlir::FloatAttr attr) {
    ...
    bool isExact;
    realVal.convertToInteger(
            intVal,
            llvm::RoundingMode::TowardZero,
            &isExact);
    return mlir::IntegerAttr::get(
            SemiringTypes::forInt(attr.getContext()),
            intVal);
}

static mlir::TypedAttr realToInt(
        mlir::TypedAttr attr,
        SemiringTypeInterface inRing,
        SemiringTypeInterface outRing) {
...
static mlir::TypedAttr tropRealtoTropInt(
        mlir::TypedAttr attr,
        SemiringTypeInterface inRing,
        SemiringTypeInterface outRing) {
    auto* ctx = attr.getContext();
    ...
    return TropIntAttr::get(
            ctx,
            outRing,
            truncateFloatAttrToInteger(floatAttr));
}
...
mlir::TypedAttr GraphAlgCast::castAttribute(
        mlir::TypedAttr attr,
        mlir::Type to) const {
    ...
    if (func) {
        return func(
                attr,
                llvm::cast<SemiringTypeInterface>(from),
