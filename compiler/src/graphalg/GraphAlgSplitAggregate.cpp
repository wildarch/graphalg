...
class GraphAlgSplitAggregate : public impl::GraphAlgSplitAggregateBase<GraphAlgSplitAggregate> {
    using impl::GraphAlgSplitAggregateBase<GraphAlgSplitAggregate>::GraphAlgSplitAggregateBase;
...
static mlir::LogicalResult decomposeMatMul(
        MatMulOp op,
        mlir::PatternRewriter& rewriter) {
    auto joinOp = rewriter.create<MatMulJoinOp>(
            op.getLoc(),
            op.getType(),
            op.getLhs(),
            op.getRhs());
...
static mlir::LogicalResult decomposeElementWiseAdd(
        ElementWiseAddOp op,
        mlir::PatternRewriter& rewriter) {
    rewriter.replaceOpWithNewOp<DeferredReduceOp>(
            op,
...
// the latter.
static mlir::LogicalResult deferReduce(
        ReduceOp op,
        mlir::PatternRewriter& rewriter) {
    rewriter.replaceOpWithNewOp<DeferredReduceOp>(
            op,
...
static mlir::LogicalResult convertMakeDense(
        MakeDenseOp op,
        mlir::PatternRewriter& rewriter) {
    auto resultType = op.getType();
    auto sring = llvm::cast<SemiringTypeInterface>(resultType.getSemiring());
    auto constantOp = rewriter.create<ConstantMatrixOp>(
            op.getLoc(),
            resultType,
            sring.addIdentity());
